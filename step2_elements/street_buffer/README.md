...
### Output:
  - A CSV file for each input point containing the estimated street buffer `width`, `None` (representing the absence of a street buffer), or `NaN` (representing failed or unreliable buffer detection).
