...
from skimage import transform # add this
...
class ATLIMG(torch.utils.data.Dataset):
    def __init__(self, datadir, split="valid"):
        # initialize self.img_files, etc.
        #self.img_files = [...]  # list of image paths
        self.img_files = sorted(glob(os.path.join(datadir, "*.jpg")))
        print(f"Found {len(self.img_files)} images in {datadir}")
    ...
    def __getitem__(self, idx):
        ...
        # Resize and process the image as usual
        ...
        # Build a target dictionary (update as needed)
